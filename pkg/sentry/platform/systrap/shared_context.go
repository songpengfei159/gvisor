...
func (q *fastPathDispatcher) loop(target *sharedContext) {
	...
	for {
		...
		if done {
			if ctx != nil {
				log.Debugf("ctx completed: %s", ctx.String())
		...
		for ctx = q.list.Front(); ctx != nil; ctx = next {
			...
			if ctx.state() == sysmsg.ContextStateNone {
				if slowPath {
					...
					// 表明需要创建并唤醒唤醒一个,并指定唤醒标识,后续判断
					log.Debugf("ctx need kick: %s", ctx.String())
			...
			log.Debugf("ctx.state() not ContextStateNone is %d", ctx.state())
			processed++
			q.list.Remove(ctx)
			if ctx == target {
				done = true
...
func (q *fastPathDispatcher) waitFor(ctx *sharedContext) syncevent.Set {
	...
	for {
		if events&sharedContextDispatch != 0 {
			log.Debugf("exec loop: %s", ctx.String())
			ctx.sync.Ack(sharedContextDispatch)
			q.loop(ctx)
			log.Debugf("exec loop over: %s", ctx.String())
		}
		events = ctx.sync.WaitAndAckAll()
		log.Debugf("events is: %d", events)
		if events&sharedContextDispatch == 0 {
			break
		}
	}
