...
restart:
	...
	if needPatch {
		log.Warningf("waitOnState2-2")
		restart, _ := s.usertrap.PatchSyscall(ctx, ac, mm)
		if restart {
			log.Warningf("waitOnState2-3")
			goto restart
		}
	}
	log.Warningf("waitOnState2-4")
	if !isSyscall && linux.Signal(c.signalInfo.Signo) == linux.SIGILL {
		log.Warningf("waitOnState2-4-1")
	...
	log.Warningf("waitOnState2-5")
	var (
		faultSP   *subprocess
		faultAddr hostarch.Addr
		faultIP   hostarch.Addr
	)
	log.Warningf("waitOnState2-6")
