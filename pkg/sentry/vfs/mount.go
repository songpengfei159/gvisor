...
func (vfs *VirtualFilesystem) NewDisconnectedMount(fs *Filesystem, root *Dentry, opts *MountOptions) *Mount {
	...
	mnt := newMount(vfs, fs, root, nil /* mntns */, opts)
	mnt.locked = opts.Locked
	return mnt
}
...
func (vfs *VirtualFilesystem) attachTreeLocked(ctx context.Context, mnt *Mount, vd VirtualDentry) error {
	...
	for pmnt := range propMnts {
		...
		if pmnt.parent().ns.Owner != mntns.Owner {
...
func (vfs *VirtualFilesystem) BindAt(ctx context.Context, creds *auth.Credentials, source, target *PathOperation, recursive bool) error {
	...
	if recursive {
		...
		if vfs.mountHasLockedChildren(sourceVd.mount, targetVd) {
...
func (vfs *VirtualFilesystem) umountTreeLocked(mnt *Mount, opts *umountRecursiveOptions) {
	...
	for _, m := range umountMnts {
		...
		if parent := m.parent(); parent != nil {
			if m.locked {
				// Restore m in parent's children list.
				parent.children[m] = struct{}{}
			} else {
				vfs.delayDecRef(vfs.disconnectLocked(m))
			}
		}
		vfs.setPropagation(m, linux.MS_PRIVATE)
	}
	vfs.mounts.seq.EndWrite()
