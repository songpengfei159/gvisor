...
func (t *Task) run(threadID uintptr) {
	...
	// interrupted.
	log.Warningf("run111111000,%d", threadID)
	t.interruptSelf()
	log.Warningf("run111111----,%d", threadID)
	for {
		...
		log.Warningf("run111111,%d", threadID)
		t.doStop()
		log.Warningf("run2222222,%d", threadID)
...
func (app *runApp) execute(t *Task) taskRunState {
	if t.interrupted() {
		...
		// tail-calling it).
		log.Warningf("runApp 0")
		return (*runInterrupt)(nil)
	}
	log.Warningf("runApp 1")
